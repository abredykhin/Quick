...
extension SceneDelegate {
    private func ensureAccount() {
        Auth.auth().rx
            .stateDidChange
            .map { _ in false }
            .catchError { _ in Auth.auth().rx.signInAnonymously().map { _ in true }}
            .bind(onNext: { newAccount in self.showScene(newAccount: newAccount) } )
            .disposed(by: disposeBag)
    }

    private func showScene(newAccount: Bool) {
